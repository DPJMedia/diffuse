...
export default function CreateOrgProjectPage() {
  ...
  const [selectedOrgIds, setSelectedOrgIds] = useState<string[]>([orgId])
  ...
  const toggleOrgSelection = (id: string) => {
    setSelectedOrgIds(prev => {
      if (prev.includes(id)) {
        return prev.filter(orgId => orgId !== id)
      } else {
        return [...prev, id]
      }
    })
                    ...
                    {allWorkspaces.length === 0 ? (
                      ...
                      <div className="max-h-[200px] overflow-y-auto space-y-2 pr-2">
                        {allWorkspaces.map((workspace) => {
                          ...
                          const isCurrentOrg = workspace.id === orgId
                          return (
                            <button
                              key={workspace.id}
                              type="button"
                            ...
                            >
                              <span className="text-body-sm text-secondary-white truncate">
                                {workspace.name}
                                {isCurrentOrg && <span className="text-cosmic-orange ml-2">(current)</span>}
                              </span>
                              {isSelected && (
                                <svg className="w-3.5 h-3.5 text-medium-gray flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
